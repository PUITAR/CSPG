...
// Files
const std::string base_vectors_path = "/var/lib/docker/anns/dataset/sift10m/base0.2m.bvecs";
const std::string queries_vectors_path = "/var/lib/docker/anns/query/sift10m/query.bvecs";
const std::string groundtruth_path = "/var/lib/docker/anns/query/sift10m/gt0.2m.ivecs";

const std::string csv_path_baseline= "output/waste/sift0.2m_hcnng_baseline.csv";
...
int main() {
  ...
  csv_baseline << "index_type,num_partition,num_random_clusters,min_size_cluters,max_mst_degree,build_time,index_size,num_queries,efq,query_time,comparison,recall,w" << std::endl;
  ...
  for (size_t efq = 10; efq <= 300; efq += 10) {
    ...
    for (size_t t = 0; t < cases; t++) {
      query_timer.Reset();
      query_timer.Start();
      dis = hcnng->GetSearchLength(nest_queries_vectors, k, efq, knn, dis);
      query_timer.Stop();
