...
const size_t part = 2;
const std::string csv_path_parallel = "output/hard/audio5w_hcnng_cspg_" + std::to_string(part) + ".csv";

// Parameters
struct params
{
  float num_random_clusters{10};
  float min_size_cluters{1000};
  float max_mst_degree{5};
  float dedup{0.5};
} default_params;
...
int main()
{
  ...
  csv_parallel << "index_type,num_partition,num_random_clusters,min_size_cluters,max_mst_degree,build_time,index_size,num_queries,k,efq,query_time,comparison,recall" << std::endl;
  ...
  for (size_t kk = 10; kk <= 100; kk += 10)
  {
    for (size_t efq = 10; efq <= 300; efq += 10)
    {
      ...
      csv_parallel << "cspg" << comma << part << comma << default_params.num_random_clusters << comma << default_params.min_size_cluters << comma << default_params.max_mst_degree << comma
                   << build_timer.GetTime() << comma << cspg->IndexSize() << comma << nq << comma << kk << comma
