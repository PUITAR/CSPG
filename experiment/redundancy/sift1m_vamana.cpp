...
const std::string csv_path = "output/redundancy/sift1m_vamana.csv";
...
int main() {
  ...
  csv << "index_type,num_partition,max_degree,alpha,efc,build_time,index_size,num_queries,efq,query_time,comparison,recall,dedup" << std::endl;
  ...
  for (float dedup = 0.1; dedup <= 0.5; dedup += 0.1) {
    ...
    index->BuildIndex(base_vectors, num_threads, {
      default_params.max_degree, default_params.alpha, default_params.efc, dedup
    });
    btimer.Stop();
