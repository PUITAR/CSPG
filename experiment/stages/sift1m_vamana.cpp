...
const std::string csv_path = "output/stages/sift1m_vamana.csv";
...
int main() {
  ...
  csv << "index_type,num_partition,max_degree,alpha,efc,build_time,index_size,num_queries,efq1,efq2,query_time,comparison,recall" << std::endl;
  ...
  index->BuildIndex(base_vectors, num_threads, {
    default_params.max_degree, default_params.alpha, default_params.efc, default_params.dedup
  });
  btimer.Stop();
  ...
  for (size_t efq1 = 1; efq1 <= 16; efq1 *= 2) {
    for (size_t efq2 = 10; efq2 <= 300; efq2 += 10) {
      ...
      csv << "cspg" << comma << part << comma << default_params.max_degree << comma << default_params.alpha << comma
