...
const std::string csv_path_baseline = "output/grid/sift1m_nsg.csv";
...
int main() {
  ...
  auto test = [&] (size_t max_degree, size_t efc) {
    ...
    build_timer.Reset();
    build_timer.Start();
    nsg->BuildIndex(base_vectors, efc);
    build_timer.Stop();
    for (size_t efq = 10; efq <= 300; efq += 10) {
      ...
      csv_baseline << "nsg" << comma << 1 << comma << max_degree << comma
        << efc << comma << build_timer.GetTime() << comma << nsg->IndexSize() << comma << nq << comma
