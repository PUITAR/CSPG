...
namespace anns
{
  ...
  namespace graph
  {
    ...
    template <typename vdim_t>
    inline const vdim_t *
    DiskANN<vdim_t>::GetDataByInternalID(id_t id) const
    {
    ...
    template <typename vdim_t>
    inline void
    DiskANN<vdim_t>::WriteDataByInternalID(id_t id, const vdim_t *data_point)
    {
    ...
    template <typename vdim_t>
    inline char *
    DiskANN<vdim_t>::GetLinkByInternalID(id_t id) const
    {
      return (char *)(data_memory_->data() + id * size_data_per_element_);
    }

    template <typename vdim_t>
    size_t DiskANN<vdim_t>::GetNumThreads()
    {
      return num_threads_;
    }

    template <typename vdim_t>
    void DiskANN<vdim_t>::SetNumThreads(size_t num_threads)
    {
      num_threads_ = num_threads;
    }

    template <typename vdim_t>
    bool DiskANN<vdim_t>::Ready() { return ready_; }
    ...
    template <typename vdim_t>
    void DiskANN<vdim_t>::Search(const vdim_t *query_data, size_t k, size_t L, std::vector<std::pair<float, id_t>> & visited)
    {
    ...
    template <typename vdim_t>
    void DiskANN<vdim_t>::Search(const vdim_t *query_data, size_t k, size_t L, id_t ep, std::vector<std::pair<float, id_t>> &visited)
    {
      assert(L >= k);
      size_t comparison = 0;
    ...
    /// @param candidates
    template <typename vdim_t>
    void DiskANN<vdim_t>::RobustPrune(
        id_t node_id,
        float alpha,
    ...
    template <typename vdim_t>
    void DiskANN<vdim_t>::BuildIndex(
...
#pragma omp parallel for schedule(dynamic, 64) num_threads(num_threads_)
    ...
    // template <typename vdim_t>
    // void DiskANN<vdim_t>::SaveDiskANN(const std::string &info_path, const std::string &edge_path, const std::string &data_path)
    // {
    //   SaveInfo(info_path);
    //   SaveData(data_path);
    //   SaveEdges(edge_path);
    ...
    template <typename vdim_t>
    size_t DiskANN<vdim_t>::GetComparisonAndClear()
    {
      return comparison_.exchange(0);
    }

    template <typename vdim_t>
    size_t DiskANN<vdim_t>::IndexSize() const
    {
    ...
    template <typename vdim_t>
    id_t DiskANN<vdim_t>::GetClosestPoint(const vdim_t *data_point)
    {
    ...
    template <typename vdim_t>
    std::vector<float> DiskANN<vdim_t>::GetSearchLength(const vdim_t *query_data, size_t k, size_t L, std::vector<std::pair<float, id_t>> &visited)
    {
    ...
    template <typename vdim_t>
    std::vector<std::vector<float>> DiskANN<vdim_t>::GetSearchLength(
        const std::vector<std::vector<vdim_t>> &queries, size_t k, size_t L,
        std::vector<std::vector<id_t>> &vids, std::vector<std::vector<float>> &dists)
    {
      const size_t nq = queries.size();
