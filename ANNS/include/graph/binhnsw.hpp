...
namespace anns
{
  ...
  namespace graph
  {
    ...
    public:
      ...
      void AddPoint(const vdim_t *data_point, float lambda)
      {
        ...
        int cur_level = GetRandomLevel(lambda);
      ...
      void Populate(const std::vector<vdim_t> &raw_data, float lambda = 0.5)
      {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_threads_)
        ...
        {
          AddPoint(raw_data.data() + i * D_, lambda);
        }

        ready_ = true;
      }

      void Populate(const std::vector<const vdim_t *> &raw_data, float lambda = 0.5)
      {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_threads_)
      ...
      int GetRandomLevel(float lambda) // sample ratio
      {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_threads_)
    ...
    template class BinHNSW<float>;
    template class BinHNSW<uint8_t>;
